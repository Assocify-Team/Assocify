...
class LoginSaveTest {
    ...
    val editor : SharedPreferences.Editor = mockk<SharedPreferences.Editor> {
    ...
    val prefs : SharedPreferences = mockk<SharedPreferences> {
        every { edit() } answers { editor }

        every { getString("user_uid", null) } answers {
            user
        }

        every { getString("association_uid", null) } answers {
            assoc
        }
    }

    val activity: MainActivity = mockk<MainActivity> {
        every { getSharedPreferences("com.github.se.assocify.PREFERENCE_FILE_KEY", Context.MODE_PRIVATE)
        } answers {
            prefs
        }
    }
