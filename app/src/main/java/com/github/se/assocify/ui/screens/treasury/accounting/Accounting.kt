...
@Composable fun Accounting() {
    ...
    val categoryMapping = mapOf(
        "Global" to listOf("Logistic Category", "Communication Category", "Game*", "ICBD", "Balelec"),
    ...
    val filteredBudgetLines = budgetLines.filter { (category, _) ->
...
fun TotalLine(totalAmount: Int) {
    ListItem(
        ...
        headlineContent = {  Text(text = "Total", style = MaterialTheme.typography.bodyLarge.copy(fontWeight = FontWeight.Bold)) },
        trailingContent =  { Text(text = "$totalAmount", style = MaterialTheme.typography.bodySmall.copy(fontWeight = FontWeight.Bold)) }
    )
}

@Composable
fun DropdownFilterChip(selectedOption1: String, options: List<String>, onOptionSelected: (String) -> Unit) {
