  ...
  fun askAssociationAccess(assocId: String, user: User, onFailure: (Exception) -> Unit){
    db.getAssociation(assocId,
      {ass ->
        ...
        val newAss = Association(uid = ass.uid, creationDate = ass.getCreationDate(), name = ass.getName(), description = ass.getDescription(), events = ass.getEvents(), members = ass.getMembers() + waitingUser, status = ass.getStatus())
        db.addAssociation(newAss, onFailure = onFailure)
      }
      , onFailure)
  }
