...
fun EditReceiptScreen() {
  ...
  Scaffold(topBar = { EditReceiptTopBar("Courses ICeLan") {} }) { innerPadding ->
    Column(
        modifier = Modifier.padding(innerPadding).verticalScroll(rememberScrollState(), true),
        verticalArrangement = Arrangement.spacedBy(15.dp, Alignment.Top),
        horizontalAlignment = Alignment.CenterHorizontally) {
          // Title
          OutlinedTextField(
              modifier =
                  Modifier.testTag("inputTodoTitle")
                      .fillMaxWidth()
              ...
              supportingText = {
                if (receiptTitle.isEmpty()) {
                  Text(
                      text = "Receipt title cannot be empty",
          ...
          OutlinedTextField(
              ...
              supportingText = {
                if (receiptDesc.isEmpty()) {
                  Text(
                      text = "Receipt desc cannot be empty",
                      color = MaterialTheme.colorScheme.error)
                }
              },
              minLines = 3)
          ...
          // Image box
          Text(text = "Receipt", style = MaterialTheme.typography.headlineSmall)
          Box(
              modifier =
          ...
          Row(
              ...
              horizontalArrangement = Arrangement.SpaceBetween) {
                TextButton(
                    onClick = { /* Cancel button action */}, modifier = Modifier.weight(1f)) {
                      Text("Cancel")
                    }

                Button(onClick = { /* Save button action */}, modifier = Modifier.weight(1f)) {
                  Text("Save")
                }
              }
        }
...
fun EditReceiptTopBar(
  ...
  CenterAlignedTopAppBar(
      title = { Text(text = receiptTitle, style = MaterialTheme.typography.headlineSmall) },
      navigationIcon = {
