...
class SnackbarSystem(private val snackbarHostState: SnackbarHostState) {
   ...
   *
   * Note : Persistent snackbars with no action will have a cross to dismiss them even if dismiss is false
   *
   ...
   * @param persistent whether the snackbar should remain until explicitely dismissed (otherwise, times out on its own)
   */
  fun showSnackbar(
      message: String,
    ...
    CoroutineScope(Dispatchers.Main).launch {
      val result = snackbarHostState.showSnackbar(message, actionLabel, dismiss || (persistent && actionLabel == null))
